...
def evaluate(ast, environment, watch=None):
    if ast["tag"] == "number":
    ...
    if ast["tag"] == "list":
        items = []
        for item in ast["items"]:
            result, item_status = evaluate(item, environment, watch)
    ...
    if ast["tag"] == "object":
        object = {}
        for item in ast["items"]:
            key, key_status = evaluate(item["key"], environment, watch)
            if key_status == "exit": return key, "exit"
            assert type(key) is str, "Object key must be a string"
            value, value_status = evaluate(item["value"], environment, watch)
    ...
    if ast["tag"] == "identifier":
        ...
        if "$parent" in environment:
            return evaluate(ast, environment, watch["$parent"])
    ...
    if ast["tag"] == "+":
        left_value, l_status = evaluate(ast["left"], environment, watch)
        if l_status == "exit": return left_value, "exit"
        right_value, r_status = evaluate(ast["right"], environment, watch)
    ...
    if ast["tag"] == "negate":
        value, status = evaluate(ast["value"], environment, watch)
    ...
    if ast["tag"] == "assert":
        if ast["condition"]:
            condition_value, cond_status = evaluate(ast["condition"], environment, watch)
            ...
            if not is_truthy(condition_value):
                ...
                if "explanation" in ast and ast["explanation"]:
                    explanation_val, expl_status = evaluate(ast["explanation"], environment, watch)
    ...
    if ast["tag"] == "if":
        ...
        if is_truthy(condition_value):
            val, status = evaluate(ast["then"], environment, watch)
            if status: # Propagate "return", "exit", "break", "continue"
                return val, status
        else:
            if "else" in ast:
                val, status = evaluate(ast["else"], environment, watch)
                if status: # Propagate "return", "exit", "break", "continue"
                    return val, status
    ...
    if ast["tag"] == "while":
        ...
        while is_truthy(condition_value):
            val, body_status = evaluate(ast["do"], environment, watch)
    ...
    if ast["tag"] == "statement_list":
        ...
        for statement in ast["statements"]:
            last_value, status = evaluate(statement, environment, watch)
    ...
    if ast["tag"] == "program":
        last_value = None
        for statement in ast["statements"]:
            val, status = evaluate(statement, environment, watch)
            if status:
                if status == "return":
    ...
    if ast["tag"] == "call":
        function, func_status = evaluate(ast["function"], environment, watch)
        if func_status == "exit": return function, "exit"
        argument_values = []
        for arg in ast["arguments"]:
            arg_val, arg_status = evaluate(arg, environment, watch)
    ...
    if ast["tag"] == "complex":
        base, base_status = evaluate(ast["base"], environment, watch)
        if base_status == "exit": return base, "exit"
        index, index_status = evaluate(ast["index"], environment, watch)
    ...
    if ast["tag"] == "assign":
        ...
        if target["tag"] == "identifier":
            ...
            base, base_status = evaluate(target["base"], environment, watch)
            if base_status == "exit": return base, "exit"
            ...
            if index_ast["tag"] == "string":
                ...
                index, index_status = evaluate(index_ast, environment, watch)
                if index_status == "exit": return index, "exit"
        ...
        value, value_status = evaluate(ast["value"], environment, watch)
        if value_status == "exit": return value, "exit"

        # WATCH FEATURE: Check if this assignment matches the watched variable
        if watch and target["tag"] == "identifier" and target["value"] == watch:
            line_info = target.get("line", "unknown")
            print("watch: " + target["value"] + " assigned to " + str(value) + " at line " + str(line_info))
    ...
    if ast["tag"] == "return":
        if "value" in ast and ast["value"] is not None: # Checks if 'return' has an expression
            evaluated_value, expression_status = evaluate(ast["value"], environment, watch)
    ...
    if ast["tag"] == "exit":
        ...
        if "value" in ast and ast["value"] is not None:
            exit_code_val, status = evaluate(ast["value"], environment, watch)
    ...
    if ast["tag"] == "import":
        filename_val, status = evaluate(ast["value"], environment, watch)
